var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
// implementations
var MachineSaleEvent = /** @class */ (function () {
    function MachineSaleEvent(_sold, _machineId) {
        this._sold = _sold;
        this._machineId = _machineId;
        console.log("generating sale event", this._sold, this._machineId);
    }
    MachineSaleEvent.prototype.machineId = function () {
        return this._machineId;
    };
    MachineSaleEvent.prototype.getSoldQuantity = function () {
        return this._sold;
    };
    MachineSaleEvent.prototype.type = function () {
        return "sale";
    };
    return MachineSaleEvent;
}());
var MachineRefillEvent = /** @class */ (function () {
    function MachineRefillEvent(_refill, _machineId) {
        this._refill = _refill;
        this._machineId = _machineId;
        console.log("generating refill event", this._refill, this._machineId);
    }
    MachineRefillEvent.prototype.machineId = function () {
        return this._machineId;
    };
    MachineRefillEvent.prototype.getRefillQuantity = function () {
        return this._refill;
    };
    MachineRefillEvent.prototype.type = function () {
        return "refill";
    };
    return MachineRefillEvent;
}());
var LowStockWarningEvent = /** @class */ (function () {
    function LowStockWarningEvent(_stock, _machineId) {
        this._stock = _stock;
        this._machineId = _machineId;
        // console.log(
        //   "stock too low warning generated: ",
        //   this._stock,
        //   this._machineId
        // );
    }
    LowStockWarningEvent.prototype.machineId = function () {
        return this._machineId;
    };
    LowStockWarningEvent.prototype.stock = function () {
        return this._stock;
    };
    LowStockWarningEvent.prototype.type = function () {
        return "low stock";
    };
    return LowStockWarningEvent;
}());
var StockLevelOkEvent = /** @class */ (function () {
    function StockLevelOkEvent(_stock, _machineId) {
        this._stock = _stock;
        this._machineId = _machineId;
        // console.log("ok stock warning generated: ", this._stock, this._machineId);
    }
    StockLevelOkEvent.prototype.machineId = function () {
        return this._machineId;
    };
    StockLevelOkEvent.prototype.stock = function () {
        return this._stock;
    };
    StockLevelOkEvent.prototype.type = function () {
        return "ok stock";
    };
    return StockLevelOkEvent;
}());
var threshold = 10;
var MachineSaleSubscriber = /** @class */ (function () {
    function MachineSaleSubscriber(machines) {
        this.machines = machines;
    }
    MachineSaleSubscriber.prototype.handle = function (event) {
        var stockEvent;
        console.log("selling ".concat(event.getSoldQuantity(), " from machine ").concat(event.machineId()));
        var machine = this.machines.find(function (machine) {
            return machine.id === event.machineId();
        });
        // prevent negative
        if (machine.stockLevel < event.getSoldQuantity()) {
            console.log("Not Enough Stock to Sell");
            return;
        }
        // check for crossing threshold
        if (machine.stockLevel >= threshold &&
            machine.stockLevel - event.getSoldQuantity() < threshold) {
            stockEvent = new LowStockWarningEvent(machine.stockLevel - event.getSoldQuantity(), event.machineId());
        }
        machine.stockLevel -= event.getSoldQuantity();
        console.log("stock level of machines:");
        this.machines.forEach(function (m) { return console.log(m.id, ":", m.stockLevel); });
        return stockEvent;
    };
    return MachineSaleSubscriber;
}());
var MachineRefillSubscriber = /** @class */ (function () {
    function MachineRefillSubscriber(machines) {
        this.machines = machines;
    }
    MachineRefillSubscriber.prototype.handle = function (event) {
        var stockEvent;
        console.log("refilling ".concat(event.getRefillQuantity(), " to machine ").concat(event.machineId()));
        var machine = this.machines.find(function (machine) {
            return machine.id === event.machineId();
        });
        // check for crossing threshold
        if (machine.stockLevel < threshold &&
            machine.stockLevel + event.getRefillQuantity() >= threshold) {
            stockEvent = new StockLevelOkEvent(machine.stockLevel + event.getRefillQuantity(), event.machineId());
        }
        machine.stockLevel += event.getRefillQuantity();
        console.log("stock level of machines:");
        this.machines.forEach(function (m) { return console.log(m.id, ":", m.stockLevel); });
        return stockEvent;
    };
    return MachineRefillSubscriber;
}());
var StockWarningSubscriber = /** @class */ (function () {
    function StockWarningSubscriber(machines) {
        this.machines = machines;
    }
    StockWarningSubscriber.prototype.handle = function (event) {
        console.log("***** Stock Levels Too Low Warning (Machine: ".concat(event.machineId(), ") *****"));
        // this.machines.find((machine) => {
        //   return machine.id === event.machineId();
        // }).stockLevel += 3;
        // console.log("stock level of machines:");
        // this.machines.forEach((m) => console.log(m.id, ":", m.stockLevel));
    };
    return StockWarningSubscriber;
}());
var StockOkSubscriber = /** @class */ (function () {
    function StockOkSubscriber(machines) {
        this.machines = machines;
    }
    StockOkSubscriber.prototype.handle = function (event) {
        console.log("***** Stock Levels OK (Machine: ".concat(event.machineId(), ") *****"));
        // this.machines.find((machine) => {
        //   return machine.id === event.machineId();
        // }).stockLevel += 3;
        // console.log("stock level of machines:");
        // this.machines.forEach((m) => console.log(m.id, ":", m.stockLevel));
    };
    return StockOkSubscriber;
}());
var PublishSubscribeService = /** @class */ (function () {
    function PublishSubscribeService(subscribers) {
        if (subscribers === void 0) { subscribers = {}; }
        this.subscribers = {};
        this.subscribers = subscribers;
    }
    PublishSubscribeService.prototype.publish = function (event) {
        var handlers = this.subscribers[event.type()];
        console.log("subscribers found: ", handlers.length);
        if (!handlers)
            return;
        for (var _i = 0, handlers_1 = handlers; _i < handlers_1.length; _i++) {
            var handler = handlers_1[_i];
            var warning = handler.handle(event);
            if (warning !== undefined) {
                return warning;
            }
        }
        return undefined;
    };
    PublishSubscribeService.prototype.subscribe = function (type, handler) {
        console.log("subscribing event:", type);
        if (!this.subscribers[type]) {
            this.subscribers[type] = [];
        }
        this.subscribers[type].push(handler);
    };
    PublishSubscribeService.prototype.unsubscribe = function (type, handler) {
        console.log("unsubscribing from event", type);
        if (!this.subscribers[type]) {
            return;
        }
        this.subscribers[type] = this.subscribers[type].filter(function (subscriber) {
            return subscriber !== handler;
        });
    };
    return PublishSubscribeService;
}());
// objects
var Machine = /** @class */ (function () {
    function Machine(id) {
        this.stockLevel = 10;
        this.id = id;
    }
    return Machine;
}());
// helpers
var randomMachine = function () {
    var random = Math.random() * 3;
    if (random < 1) {
        return "001";
    }
    else if (random < 2) {
        return "002";
    }
    return "003";
};
var eventGenerator = function () {
    var random = Math.random();
    if (random < 0.5) {
        var saleQty = Math.random() < 0.5 ? 1 : 2; // 1 or 2
        return new MachineSaleEvent(saleQty, randomMachine());
    }
    var refillQty = Math.random() < 0.5 ? 3 : 5; // 3 or 5
    return new MachineRefillEvent(refillQty, randomMachine());
};
// program
(function () { return __awaiter(_this, void 0, void 0, function () {
    var machines, saleSubscriber, refillSubscriber, warningSubscriber, okSubscriber, pubSubService, events, _i, events_1, event_1, result, _a, events_2, event_2, result;
    return __generator(this, function (_b) {
        machines = [
            new Machine("001"),
            new Machine("002"),
            new Machine("003"),
        ];
        saleSubscriber = new MachineSaleSubscriber(machines);
        refillSubscriber = new MachineRefillSubscriber(machines);
        warningSubscriber = new StockWarningSubscriber(machines);
        okSubscriber = new StockOkSubscriber(machines);
        console.log();
        pubSubService = new PublishSubscribeService();
        console.log("-------------------- Subscribing to Events --------------------");
        pubSubService.subscribe("sale", saleSubscriber);
        pubSubService.subscribe("refill", refillSubscriber);
        pubSubService.subscribe("low stock", warningSubscriber);
        pubSubService.subscribe("ok stock", okSubscriber);
        // create 5 random events
        console.log("-------------------- Generating Random Events --------------------");
        events = [1, 2, 3, 4, 5].map(function () { return eventGenerator(); });
        // publish the events
        // console.log("-------------------- Publishing Events --------------------");
        // events.map((event) => pubSubService.publish(event));
        console.log("-------------------- Publishing Events --------------------");
        for (_i = 0, events_1 = events; _i < events_1.length; _i++) {
            event_1 = events_1[_i];
            result = pubSubService.publish(event_1);
            if (result) {
                // pubSubService.publish(result);
                events.push(result);
            }
        }
        // unsubscribing
        console.log("-------------------- Unsubscribing --------------------");
        pubSubService.unsubscribe("sale", saleSubscriber);
        // create 5 random events
        console.log("-------------------- Generating Random Events --------------------");
        events = [1, 2, 3, 4, 5].map(function () { return eventGenerator(); });
        // publish the events
        console.log("-------------------- Publishing Events --------------------");
        for (_a = 0, events_2 = events; _a < events_2.length; _a++) {
            event_2 = events_2[_a];
            result = pubSubService.publish(event_2);
            if (result) {
                // pubSubService.publish(result);
                events.push(result);
            }
        }
        return [2 /*return*/];
    });
}); })();
